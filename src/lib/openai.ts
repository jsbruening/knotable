...
const createOpenAIClient = () => {
  ...
  if (apiBase) {
    return new OpenAI({
      apiKey,
      baseURL: `${apiBase}/openai/deployments/gpt-4`,
...
export async function generateCampaignContent(prompt: string) {
  try {
    const completion = await openai.chat.completions.create({
      model: process.env.OPENAI_API_BASE ? 'gpt-4' : 'gpt-4',
      messages: [
        {
          role: 'system',
          content: 'You are an expert educational content creator specializing in Bloom\'s Taxonomy. Create engaging, structured learning content.',
        },
        {
          role: 'user',
          content: prompt,
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert quiz creator. Always respond with valid JSON format.',
        },
        {
          role: 'user',
          content: prompt,
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert educational content creator. Always respond with valid JSON format.',
        },
        {
          role: 'user',
          content: prompt,
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert educational resource curator. Always respond with valid JSON format.',
        },
        {
          role: 'user',
          content: prompt,
